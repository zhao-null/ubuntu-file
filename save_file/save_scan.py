...
class dataRecorder:
	def __init__(self):
		...
		self.filterScanSubscriber = rospy.Subscriber('slave/filterScan', LaserScan,self.filterScanCallback,queue_size=1)
		self.objectScanSubscriber = rospy.Subscriber('slave/filterScan', LaserScan,self.objectScanCallback,queue_size=1)
	...
	def objectScanCallback(self,scan_data):
		self.objectRanges = np.array(scan_data.ranges)
	...
	try:
		...
		if(rospy.is_shutdown()):
			...
			for i in range(len(recoder.ranges)):
				...
				if(recoder.objectRanges[i]!=0):
					x2.append(recoder.objectRanges[i] * math.cos(recoder.angle_min + i*recoder.angle_increment))
					y2.append(recoder.objectRanges[i] * math.sin(recoder.angle_min + i*recoder.angle_increment))
