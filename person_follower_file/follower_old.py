...
class Follower:
	...
	def positionUpdateCallback(self, position):
		...
		if(angularSpeed>-0.002 and angularSpeed<0.002):
			angularSpeed = 0
		if(self.initFlag):
			linearSpeed = np.clip(-uncliped_lin_speed, -self.max_init_speed, self.max_init_speed)
		else:
			linearSpeed  = np.clip(-uncliped_lin_speed, -self.max_speed, self.max_speed)
		if(linearSpeed>-0.002 and linearSpeed<0.002):
			linearSpeed = 0
		...
		if(self.initFlag):
			if(abs(distance-self.targetDis)>0 and abs(distance-self.targetDis)<0.05 and abs(angleX - self.targetAngle)>0 and abs(angleX - self.targetAngle)<0.02):
				self.initFlag = 0
				rospy.loginfo('PID initiate is done!!!')
				self.PID_controller.ParamsToZero([self.targetAngle, self.targetDis])
