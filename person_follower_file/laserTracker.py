...
class laserTracker:
	...
	def posePubSet(self,): # 将符合距离长度的直线放入objectPoseArray集合中
		...
		if(self.lastAngle ==0 ):
			...
			self.angle_mid = (self.angle_sum / self.count) *0.6 + self.lastAngle * 0.4
	...
	def polyContourFit(self,start,end,angle_min,angle_delta,Eps): # 提取直线和角点
		...
		if(length>0.12 and length<0.24 and sumLen>1 and sumLen<1.5 and sumDis<8 and sumDis>2): # 0.18 1.2 5
			self.posePubSet() # 确定目标位置和角度
		# else:
		# 	if(self.angle_sum / self.count<0.5 and self.angle_sum / self.count>-0.5 ):
	...
	def registerScan(self, scan_data):
		...
		for index in self.startEndIndex:
			midIndex = int((index[1]-index[0])/4)
			for i in range(int(index[0]),int(index[1]+1)):
				if(self.ranges[i] != float('inf')):
					...
					if(i>(index[0]+midIndex) and i<(index[1]-midIndex)):
