...
class simplePID:
	...
	def __init__(self, target, P, I, D):
		...
		self.dis_err = 0.5
		self.speed = 0.4
		self.x_qual = np.arange(-self.dis, self.dis, 0.001)
		...
		self.angle_x = np.arange(-0.4, 0.4, 0.0002)
		self.angle_x_e = np.arange(-0.2, 0.2, 0.01)
		self.angle_output  = np.arange(-1, 1, 0.001)
		# 定义模糊控制变量
		self.angle = ctrl.Antecedent(self.angle_x, 'angle')
		self.ang_error = ctrl.Antecedent(self.angle_x_e, 'ang_error')
		self.a_output = ctrl.Consequent(self.angle_output, 'a_output')
		# 生成模糊隶属函数
		self.angle['L'] = fuzz.trimf(self.angle_x, [-0.4, -0.4, 0])  #定义质量差时的三角隶属度函数横坐标
		self.angle['M'] = fuzz.trimf(self.angle_x, [-0.4, 0, 0.4])
		self.angle['H'] = fuzz.trimf(self.angle_x, [0, 0.4, 0.4])

		self.ang_error['L'] = fuzz.trimf(self.angle_x_e, [-0.2, -0.2, 0]) #定义服务差时的三角隶属度函数横坐标
		self.ang_error['M'] = fuzz.trimf(self.angle_x_e, [-0.2, 0, 0.2])
		self.ang_error['H'] = fuzz.trimf(self.angle_x_e, [0, 0.2, 0.2])

		self.a_output['L'] = fuzz.trimf(self.angle_output, [-1, -1, 0]) #定义小费的三角隶属度函数横坐标
		self.a_output['M'] = fuzz.trimf(self.angle_output, [-1, 0, 1])
		self.a_output['H'] = fuzz.trimf(self.angle_output, [0, 1, 1])

		self.a_output.defuzzify_method='centroid'
		#可视化这些输入输出和隶属函数

		#规则
		self.a_rule1=ctrl.Rule(antecedent=((self.angle['L'] & self.ang_error['L'])|(self.angle['L'] & self.ang_error['M'])|(self.angle['M'] & self.ang_error['L'])),consequent=self.a_output['L'],label='Low')
		self.a_rule2=ctrl.Rule(antecedent=((self.angle['M']&self.ang_error['M'])|(self.angle['L']&self.ang_error['H'])|(self.angle['H']&self.ang_error['L'])),consequent=self.a_output['M'],label='Medium')
		self.a_rule3=ctrl.Rule(antecedent=((self.angle['M']&self.ang_error['H'])|(self.angle['H']&self.ang_error['M'])|(self.angle['H']&self.ang_error['H'])),consequent=self.a_output['H'],label='High')

		# rule2.view()
		self.ang_outputping_ctrl = ctrl.ControlSystem([self.a_rule1, self.a_rule2, self.a_rule3])
		self.ang_outputping = ctrl.ControlSystemSimulation(self.ang_outputping_ctrl)
	...
	def update(self, current_value):
		...
		self.ang_outputping.input['angle'] = error[0]
		self.ang_outputping.input['ang_error'] = 0 #error[0] - self.last_error[0]
		self.ang_outputping.compute()
		self.OutAngle = self.ang_outputping.output['a_output']
